...
| **Record layout** | The delivered file's own schema: one row per field with its name, data type and size, uploaded as a **fourth artifact** and the only optional one (ADR-060). Its field name is what appears as the DIRT column. Uploaded per run and remembered per configuration — promoted at finalize, borrowed by a later run that uploads none, and every finding resting on a borrowed one names the run and the date it came from. |
| **Attribute** | A field / column in the delivered data. Names can differ across OSL, config, and reports → the **attribute dictionary** (ADR-062), and the older **attribute alias** table, which is still read beside it. |
| **Product code** | A name for a set of attributes delivered together, so an OSL can say *"all attributes from ABC"* instead of listing them (ADR-061). Reference data an administrator enters. The model reads only that a requirement names a code; **code** looks up what it contains and checks that it exists — an undefined code is a finding and never an empty expansion. |
...
| **Re-check** | Re-running stages 5–7 after a rule or trace edit. Seconds, no LLM. **Queued by the edit itself** — there is no button, because one that re-ran the comparisons against inputs nobody had changed said nothing and showed nothing (Phase 6.23b). It leaves the run in `needs_review`, so the run reports that one is in flight and the screen follows it; refused on any run that is not awaiting review, so a frozen report cannot be rewritten (ADR-066). A re-check that fails leaves the run reviewable: the findings it had are still there. |
...
| **Report chat** | An optional read-only panel on a frozen report that answers questions about that one run (Phase 8). Off until an administrator turns it on. It reads a **context pack** code builds; it cannot compare, compute, decide, change anything, or see another run; the prose streams and every citation is checked against the pack before it is shown. Nothing is stored — not the question, not the answer, not the transcript. |
| **Context pack** | Everything the report chat may see about one run, assembled on the server from the run id on every turn and never accepted from the client. Findings (shadow excluded), coverage and the attestation, the rules in force, the programme's rules, this run's guidance, the last three finalized runs of that configuration, an inventory of the artifacts, and the frozen report's own text. Capped as a whole, and it says what it trimmed. |
| **Citation** (chat) | An identifier beneath a chat answer — `F-003`, `R-012` — that resolved against the pack. One that does not resolve is discarded before the answer is shown, never dimmed and never rendered. When the model said nothing about what its answer rests on, the panel says the citations could not be verified and the answer still stands. |
...
| **The ladder** | The one way every name in the product is resolved (ADR-054): exact, separators-as-noise, the same words, an **alternate** somebody wrote down, and — only where all four failed — the model. **A rung that ties is a rung that failed**: two candidates matching equally well is not an answer, so the ladder falls through rather than guessing. |
| **Attribute dictionary** | The ladder's fourth rung for attribute names, as tables (ADR-062): one **term** per attribute and one **spelling** per way an artifact writes it, with provenance on the spelling. One name means one attribute, so a spelling another term already claims is refused. A spelling with no artifact is offered everywhere, which is the ordinary case. |
| **Attribute suggestion** | What a delivery appears to call an attribute the tool could not locate — read out of the **record layout** in code at no model call where it declares exactly one resembling field, and from the ladder's fifth rung where it does not (Phase 6.22f). Nothing is in force until somebody accepts it; accepting writes a dictionary spelling, and from then the fourth rung resolves the name and no call is made. |
| **Attribute lookup cap** | How many times one run may ask the model which column an attribute is (`llm.max_attribute_calls_per_run`). A **soft** limit inside the token ceiling: past it the run stops asking and names what it did not look for, and nothing is refused. It should fall to zero as the dictionary fills. |
