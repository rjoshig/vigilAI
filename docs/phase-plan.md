...
| 6.19 | Say what helps, and teach it in the product — a marker on every field that reaches the model or decides what it is shown, what a failed run will not say, usage counted per person, and a Guide in each app's sidebar written for the person in front of it | days | 🟡 **parts A and C complete** — every field says what it does, a failure says where it happened, and usage is counted per person; the in-app Guide is not started | [phase-6.19.md](phase-6.19.md) |
| 6.20 | Three roles — user, reviewer and administrator — with capabilities rather than role checks, several roles per account, and each console showing a person only their own job | days | 🟡 **in progress** — the capability table and the per-account roles are built; the placeholder holds user and admin. The API guards and both consoles are not | [phase-6.20.md](phase-6.20.md) |
