...
| 6.22 | Product codes, the record layout, and attribute resolution — the delivered file's record schema uploaded as a fourth artifact and verified against the OSL; a product code the OSL can name instead of listing every attribute, expanded in code; and one central dictionary saying what each attribute is called in the OSL, the configuration and each report, so anywhere the tool looks for a row, cell or column it consults one reference | 2–3 weeks | 🟡 **in progress** — 6.22a complete (ADR-059): an attribute name code cannot resolve is `could_not_evaluate`, not a high-severity violation. Measured on `attribute_renamed`: one high finding before, zero after | [phase-6.22.md](phase-6.22.md) |
