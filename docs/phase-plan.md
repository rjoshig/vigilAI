...
| 6.15 | A compliance rule should survive being spelled differently — a compliance rule is a substring match on config JSON paths, and one extra level of nesting turns three correct controls into three high-severity findings that are wrong. Measured, specified, and the approach deliberately left open | TBD | ⬜ **specified, not started** | [phase-6.15.md](phase-6.15.md) |
