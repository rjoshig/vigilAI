...
| 6.14 | The artifacts belong together — an artifact match check that compares the submitted configuration id, customer and credit date against what the artifacts declare, before any model call, and holds the run until a person accepts each mismatch with a reason; the credit date resolved by scoped label and compared by value; one register and one marker for every field that reaches the model; tooltips on by default; the theme locked by default | 2–3 weeks | ✅ **complete** — the live cap countdown was carried into 6.17b and built there | [phase-6.14.md](phase-6.14.md) |
...
| 6.17 | What is left, gathered in one place — the programme keyword check is the one surface never measured for the brittleness two others were; plus the deferred cap countdown, how scope reaches the compliance locator, and the rollout plan and training documents that have fallen behind | days | ✅ **complete** — 6.17a measured (brittle, and it fails at HIGH) and repaired; the cap countdown and the locator's scope followed | [phase-6.17.md](phase-6.17.md) |
...
| 6.22 | Product codes, the record layout, and attribute resolution — the delivered file's record schema uploaded as a fourth artifact and verified against the OSL; a product code the OSL can name instead of listing every attribute, expanded in code; and one central dictionary saying what each attribute is called in the OSL, the configuration and each report, so anywhere the tool looks for a row, cell or column it consults one reference | 2–3 weeks | 🟡 **in progress** — a, b, d, e, f and g complete and c built (ADR-059 to ADR-062). An attribute name code cannot resolve is `could_not_evaluate` rather than a high-severity violation; the record layout is a fourth artifact, optional, promoted per configuration; a product code is expanded in code and an undefined one fails rather than expanding to nothing; the dictionary is the ladder's fourth rung, with a soft cap on what the fifth may cost; and a run proposes what the delivery calls each attribute it could not locate, which a person accepts. Two items stay open in c and say so under its heading: a bulk master-file upload, how a code is recognised in real OSL prose (phase 7's to tune), and — found by audit on 2026-09-21 — **no screen in the admin console for product codes, the dictionary or attribute suggestions**, which also moves d to amber | [phase-6.22.md](phase-6.22.md) |
| 6.23 | The draft that can be finished, and the button that did nothing — cloning a run already creates a draft and nothing can advance it, so the New run form learns to finish one; a draft expires on a window an administrator sets; the standalone Re-check control, which is documented nowhere and shows the user nothing, is removed; and the runs list opens a frozen report in one click | 2 weeks | ✅ **complete** (ADR-063 to ADR-067): the closed status set and its guards, the New run form finishing a draft, a five-day window an administrator sets — with `retention.days` and `uploads.max_mb` finally read at all — the Re-check button gone and the edit that queues a re-check built at last, visible while it runs, and a Report column carrying the frozen verdict | [phase-6.23.md](phase-6.23.md) |
...
| 8 | Ask the frozen report — an optional chat box on the final report page, aware of one run and nothing else: the global rules in force, the previous run of that configuration id, the findings of the last three runs, this run's findings, and an inventory of its artifacts. The prose streams; the citations are checked before they are shown; an administrator turns it on and tunes it from a Chat section in the console | 2–3 weeks | 🟡 **built** (ADR-068 to ADR-070) — the pack, the streamed answer with checked citations, the panel, the Chat console section and the aggregates switch. The release tag is cut and pushed: `v0.6.23` on `c392f6c` before the chat, `v0.8.0` on `31c6fae` with it. **One thing outstanding:** what a real model does when asked to compute, to stray outside the pack, or to change something — a mock answers what it was scripted to, so it needs a real endpoint | [phase-8.md](phase-8.md) |
