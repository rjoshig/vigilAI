...
| 6.23 | The draft that can be finished, and the button that did nothing — cloning a run already creates a draft and nothing can advance it, so the New run form learns to finish one; a draft expires on a window an administrator sets; the standalone Re-check control, which is documented nowhere and shows the user nothing, is removed; and the runs list opens a frozen report in one click | 2 weeks | 🟡 **in progress** — 6.23a complete (ADR-063, ADR-066): the closed status set, one guard that stops a re-check rewriting a finalized run, a finalize gate that stops calling an empty draft ready to freeze, and the run summary a re-check used to erase | [phase-6.23.md](phase-6.23.md) |
