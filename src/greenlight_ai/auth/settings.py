...
class AuthSettings(BaseModel):
    ...
    def from_env(cls, environ: Mapping[str, str] | None = None) -> AuthSettings:
        ...
        settings = cls(
            admin_auth=_flag(source, "GREENLIGHT_AI_ADMIN_AUTH"),
            user_auth=_flag(source, "GREENLIGHT_AI_USER_AUTH"),
            session_ttl_s=int(source.get("GREENLIGHT_AI_SESSION_TTL_S", str(8 * 60 * 60))),
            idle_ttl_s=int(source.get("GREENLIGHT_AI_SESSION_IDLE_S", str(60 * 60))),
            min_password_length=int(source.get("GREENLIGHT_AI_MIN_PASSWORD_LENGTH", "12")),
            lockout_threshold=int(source.get("GREENLIGHT_AI_LOCKOUT_THRESHOLD", "5")),
            lockout_s=int(source.get("GREENLIGHT_AI_LOCKOUT_S", str(15 * 60))),
            bind_host=source.get("GREENLIGHT_AI_BIND_HOST", "127.0.0.1").strip() or "127.0.0.1",
        )
