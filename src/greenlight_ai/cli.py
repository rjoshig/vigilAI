...
``greenlight-ai run`` takes the three inputs and writes findings JSON. It is the first entry
...
Example:
    greenlight-ai run --osl osl.docx --config config.json \\
...
from greenlight_ai.llm.settings import ConfigError, LLMSettings
...
_LOG: Final = logging.getLogger("greenlight_ai")
