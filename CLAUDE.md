...
| 6.19 | Say what helps, and teach it in the product: the "Helps the AI" marker on every field that reaches the model or decides what it is shown, what a failed run will not say, usage counted per person, and a Guide in each app's sidebar | `docs/phase-6.19.md` | 🟡 parts A and C complete; the in-app Guide not started |
| 6.20 | Three roles — user, reviewer, admin — with capabilities rather than role checks, several roles per person, and a console that shows each person only their own job | `docs/phase-6.20.md` | 🟡 in progress — 6.20a built and 6.20b mostly built (roles stored per account; the placeholder is user and admin). Nothing is enforced yet |
