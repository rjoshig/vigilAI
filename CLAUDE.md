...
| 6.14 | The artifacts belong together: an artifact match check before any model call (configuration id, customer, credit date) any submitter accepts with a reason; the credit date resolved by label; every field says what it does to a run; tooltips on by default; the theme locked | `docs/phase-6.14.md` | ✅ complete (the cap countdown was built as 6.17b) |
...
| 6.17 | What is left, gathered in one place: measure the programme keyword check, the live cap countdown, how scope reaches the compliance locator, and the two standing touchpoints that have fallen behind | `docs/phase-6.17.md` | ✅ complete |
...
| 6.22 | Product codes, the record layout, and attribute resolution: the delivered file's schema as a fourth artifact; a product code the OSL can name instead of listing attributes; one dictionary of what each attribute is called in each artifact | `docs/phase-6.22.md` | 🟡 in progress — **a, b, e, f, g complete; c and d have no admin screen** (a bulk master-file upload, and how a code reads in real OSL prose, which is phase 7's) |
| 6.23 | The draft that can be finished, and the button that did nothing: a clone leaves an editable draft the New run form completes, drafts expire on a console-set window, the undocumented Re-check control goes | `docs/phase-6.23.md` | ✅ complete |
...
- `src/greenlight_ai/` subpackages: `parsers/ rules/ pipeline/ checks/ llm/ db/ api/ worker/
  report/ auth/ config/ training/ meaning/` + `cli.py`. Dependencies point down; `api/` never imports a `pipeline/` **stage** (`run.py`, `sN_*.py`) and may import only the pure leaves ADR-071 names — `tests/test_architecture.py` enforces it.
